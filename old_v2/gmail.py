...
def get_unread_emails() -> str:
    ...
    try:
        ...
        # Use LLM to summarize emails intelligently
        emails_text = f"You have {count} unread email{'s' if count != 1 else ''}:\n\n"
        for i, e in enumerate(emails, 1):
            emails_text += f"{i}. From: {e['sender']}\n   Subject: {e['subject']}\n   Preview: {e['snippet']}\n\n"
        
        summary_prompt = f"""Summarize these unread emails in a conversational, spoken-friendly way. 
Be concise and highlight important information. Include sender and subject.

{emails_text}

Provide a natural summary as if speaking to the user."""
        ...
        try:
            ...
            print(f"LLM email summarization failed: {llm_error}")
            count = len(emails)
            parts = [f"You have {count} unread email{'s' if count != 1 else ''}."]
            for i, e in enumerate(emails, 1):
                snippet_part = f" {e['snippet']}" if e["snippet"] else ""
                parts.append(f"Email {i}: From {e['sender']}, Subject: {e['subject']}.{snippet_part}")
